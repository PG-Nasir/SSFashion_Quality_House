		...
		try {
			...
			if(sizeList.trim().length()>0) {
				...
				for (String item : sizeLists) {
					...
					styleName = itemProperty[4].substring(itemProperty[4].indexOf(":")+1).trim();
					...
					qcPassedType = 1;
					list.add(new AccessoriesSize(autoId, transectionId, purchaseOrder, styleId,styleName, itemId,itemName,itemColorId,itemColor, fabricsId,fabricsName, fabricsColorId,fabricsColorName, sizeId ,supplierSizeId, unitId,"unitName", balanceQty,unitQty, rackName, binName,qcPassedType));
				}
