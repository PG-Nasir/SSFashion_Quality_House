window.onload = ()=>{
	document.title = "General Store Item Returned";
	calculate();
  } 
...
function setGeneralReceivedInvoice(invoiceNo,type){
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: './GeneralReturnedInvoiceInfodata',
        data: {
        invoiceNo:invoiceNo,
        type:type
        },
	...
	if (supplierId != '0') {
		if (date != '0') {
		    $.ajax({
		        type: 'POST',
		        dataType: 'json',
		        url: './confrimGeneralReturnedItem',
		        data: {
		        invoiceNo:invoiceNo,
